...
object MapFlatMap:
  ...
  object library:
    ...
    // Hint: you want an extension that wraps a generic `M[A]` type class
    ...
    extension [M[_], A] (instance: Map[M])
      def map[B](f: A => B): M[B] =
        sys.error("unimplemented")
    ...
    extension [M[_], A](instance: FlatMap[M])
