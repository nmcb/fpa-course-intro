...
object Prolog:
  ...
  def unify(lhs: Term, rhs: Term)(acc: Option[Env])(implicit T: Subst[Term]): Option[Env] =
    ...
    acc match
      case None          => Option.empty[Env]
      case env@Some(cur) => (T.subst(cur)(lhs), T.subst(cur)(rhs)) match
          case (Var(x), y)                                                  => Some(Env(cur.toMap + (x -> y)))
          case (x, Var(y))                                                  => Some(Env(cur.toMap + (y -> x)))
          case (Fun(x, xs), Fun(y, ys)) if x == y && xs.length == ys.length => xs.zip(ys).foldRight[Option[Env]](env) { (q, w) => unify(q._1, q._2)(w)(using T) }
          case _                                                            => None
  ...
  def solve(rules: List[Rule], env: Option[Env], tags: List[TaggedTerm])(implicit L: Taggable[List[Rule]], T: Subst[Term]): Result =
    (rules, env, tags).runtimeChecked match
      case (_, None, _)      => ApplyRules(List.empty[ApplyRule])
      case (_, Some(e), Nil) => Done(e)
